...
# In this implementation, character is separated into two parts:
# username and char_id, with delimiter '-'
# Expected char value is an 8 digit integer + '-' + username
# Character string is created in .util.py (in /api)
# Note additional 'username field' used from request body.
# See build_params & load_character within util
 
def load_character(character, name1, name2, instruct=False):
    context = greeting = turn_template = ""
    greeting_field = 'greeting'
    picture = None
    if character not in ["Alpaca", "Vicuna"]:  # Removed extra closing parentheses
        username, char_id = character.split('-')
        print("Assigning char_id_two")
        char_id_two = char_id
    else:
        char_id = ""
        username = character
    ...
    if character not in ['None', '', None]:
        ...
        for extension in ["yml", "yaml", "json"]:
            # folder represents the 'characters' folder
            if username not in ['Alpaca', 'Vicuna']:
                filepath = Path(f'{folder}/{username}/{char_id_two}/{char_id}.{extension}')
                print(filepath)
            else:
                print(f"{username} detected. Adjusting filepath to Vicuna-v1.1...")
                filepath = Path(f'{folder}/{username}.yaml')
            if filepath.exists():
                break

        if filepath is None:
            logger.error(f"Could not find character file for {char_id} in {folder}/{username} folder. Please check your spelling.")
        ...
        for k in ['name', 'bot', '', 'char_name']:
            if k in data and data[k] != '':
                name2 = data[k]
                break

        # Find the user name (if any)
        for k in ['your_name', 'user', '']:
            if k in data and data[k] != '':
                name1 = data[k]
                break
